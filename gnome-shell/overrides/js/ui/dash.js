...
class DashIcon extends AppDisplay.AppIcon {
    ...
    activate(button) {
        ...
        const isTouchEvent = eventType === Clutter.EventType.TOUCH_BEGIN ||
        ...
        const isMiddleButton =
            effectiveButton && effectiveButton === Clutter.BUTTON_MIDDLE;
        const isCtrlPressed =
        ...
        if (openNewWindow) {
            ...
            const windows = this.app.get_windows().filter(metaWindow => {
            ...
            if (windows.length > 1) {
                ...
                const focusedIndex = windows.findIndex(metaWindow =>
                    metaWindow === focusedWindow);
                const nextWindow = focusedIndex >= 0
